...
## 0-A. 해결됨 (2026-08-29 낮, 0.1.3) — "아이콘이 저절로 사라졌다 돌아온다"

**범인은 화면 기록 표시등(`sys:AudioVideoModule`)이었다.** 제어 센터가 상황에 따라 넣었다
뺐다 하고 위치도 macOS가 정해서 Fire가 못 옮긴다. 메뉴바가 꽉 찬 상태에서 이게 뜨면
맨 왼쪽이 노치에 닿아 사라진다. **45초 관찰에서 11↔12개 요동을 실측.**

```
노치 오른쪽 645pt | 지정 12개 622pt | 여유 23pt | 표시등 38pt → 660pt 초과
```

### 왜 구분자를 옮겨서는 못 고치나

**구분자의 위치값은 좌표가 아니라 순서다.** 값이 565로 고정인데 실제로 그려지는 x는
850↔878로 움직이는 것을 실측했다. macOS가 항목을 오른쪽부터 빈틈없이 채우므로, 구분자의
렌더 위치는 **그 오른쪽 항목들의 총 폭**이 정한다. 좌표에 못 박을 수단이 없다.

### 고친 구조

`FireKit.MenuBarCapacity`(순수 로직, 테스트 7개)가 폭을 계산한다.

1. 가용 폭 = 화면 오른쪽 끝 − 노치 오른쪽 끝 (`auxiliaryTopRightArea.minX`)
2. 여유분 = **이 기계에서 실제로 관측된** 들락날락 항목의 폭
   (`lastKnownItems` 중 지금 없는 `sys:`/제어센터 항목의 최대 폭). 상수를 쓰지 않는다
3. 오른쪽부터 누적해 `가용 − 여유`를 넘기 직전까지만 남기고, 넘치는 것은 **Fire가 먼저 접는다**
4. 접힌 것은 `fireBarItems`에 포함되어 Fire Bar에서 꺼내 쓸 수 있다

이러면 노치까지 밀려나는 항목이 없어 macOS가 개입할 일이 사라진다.

```
             항목 수     최좌단        앱 아이콘
수정 전      11 ↔ 12    850 ↔ 878    사라졌다 돌아옴
수정 후      10 ↔ 11    878 ↔ 926    40초 내내 동일
```

수정 후의 개수 변화는 **표시등 자체**가 예약된 자리에 들어왔다 나가는 것이다. 정상이다.

### 곁들여

- **위치값 오염** — 구분자에 밀려 화면 밖으로 나간 항목의 좌표를 macOS가 저장해버린다.
  HiddenNotch 2430, Workspace Shelf 2280, Shottr 2557(전부 화면 폭 1470 초과).
  이 값이 남으면 그 항목은 영원히 맨 왼쪽에 박힌다. Fire는 자기 것만 걸러내고 있었다
  (`sanitizeStoredPositions`). 우리 앱 둘은 `defaults write` + 재실행으로 고쳤다.
  **남의 앱은 그 앱이 실행 중이면 못 고친다** — 이게 남은 한계다.
- **「잠시 모두 펼치기」** — 숨긴 아이콘은 화면 밖이라 `⌘`+드래그로 잡을 수 없다.
  "드래그하세요"라고 안내해놓고 대상이 안 보이던 모순을 없앴다. `isArrangeMode`.
- **설정 화면 용량 표시**, **안 보이는 이유 구분**(`capacityOverflowIds`)
- **⌘+드래그 안내 버그** — 노치에 가려진 항목의 못 믿을 좌표(화면 1470인데 x=2722)를
  기준으로 삼아 "보이는 것 10개를 전부 옮기세요"가 나왔다(「제어 센터」만 5줄 반복).
  좌표를 믿을 수 있는 항목만 계산에 넣는다.

### ⚠️ 로컬 빌드 함정

`./build.sh` 기본값은 Apple Development인데 설치본이 Developer ID(릴리즈본)이면 TCC가
다른 앱으로 보고 **권한이 끊긴다.** 반드시:

```bash
FIRE_SIGN_IDENTITY="Developer ID Application: RRLLAB (D9FZ6BL5FD)" ./build.sh release
rm -rf /Applications/Fire.app && ditto .build/app/Fire.app /Applications/Fire.app
```

`/Applications`은 덮어쓰기가 안 된다(앱 관리 권한). 지우고 새로 넣어야 한다.
